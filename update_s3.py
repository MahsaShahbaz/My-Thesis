...
class LocalUpdate(object):
    def __init__(self, args, dataset, u_id, idxs):
        ...
        #logging.info(f"User {self.u_id} dataset size: {self.dataset_size}")

        # Calculate and log sample rate
        self.sample_rate = self.args.local_bs / self.dataset_size
        #logging.info(f"User {self.u_id} sample rate: {self.sample_rate:.5f}")
    ...
    def update_weights(self, model, global_round, u_step=0):
        ...
        for iter in range(self.args.local_ep):            
            ...
            #logging.info(f'User {self.u_id}, Global Round {global_round}, Local Epoch {iter}, Loss: {epoch_loss[-1]}')
